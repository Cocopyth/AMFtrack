...
def get_hulls(exp,ts):
    ...
    for t in ts:
        ...
        if len(selected)>=0:
            area_max = 0
            for g in selected:
                nodes = np.array([node.pos(t) for node in exp.nodes if node.is_in(t) and
                                  np.all(is_in_study_zone(node,t,1000,150)) and (node.label in g.nodes)])
...
def get_nodes_in_ring(hull1,hull2,t,exp):
    nodes = [node for node in exp.nodes if node.is_in(t) and hull2.contains(Point(node.pos(t))) and not
    hull1.contains(Point(node.pos(t))) and np.all(is_in_study_zone(node,t,1000,200))]
    return(nodes)

def get_hyphae_in_ring(hull1,hull2,t,exp):
    hyphae = [hyph for hyph in exp.hyphaes if hyph.end.is_in(t) and hull2.contains(Point(hyph.end.pos(t))) and not
    hull1.contains(Point(hyph.end.pos(t))) and np.all(is_in_study_zone(hyph.end,t,1000,200))]
    return(hyphae)
...
def get_BAS_length_in_ring(hull1,hull2,t,exp,op_id):
    ...
    time_plate_info, global_hypha_info, time_hypha_info = get_data_tables(op_id,redownload=False)
    ...
    bas_ring = select_time.loc[(select_time['end'].isin(bas))&(select_time['end'].isin(hyphae_ring))&(select_time['timestep']==t)]
...
def get_rate_branch_in_ring(hull1,hull2,t,exp,rh_only):
    growing_tips = get_growing_tips(hull1,hull2,t,exp,rh_only)
    new_tips = [tip for tip in growing_tips
                 if tip.ts()[0] == t]
    timedelta = get_time(exp, t, t+1)
...
def get_rate_stop_in_ring(hull1,hull2,t,exp,rh_only):
    growing_tips = get_growing_tips(hull1,hull2,t,exp,rh_only)
    stop_tips = [tip for tip in growing_tips
...
def get_regular_hulls_area_fixed(exp,ts,incr):
    path = directory_scratch + \
           f"temp/hulls_{exp.plate}_{incr}_" \
