...
from amftrack.pipeline.functions.transport_processing.high_mag_videos.register_videos import add_attribute

hyphae = pd.read_excel("/home/cbisot/pycode/AMFtrack/amftrack/notebooks/transport/hyphae.xlsx")
...
def add_betweenness_QP(exp,t):
    ...
    for edge in exp.nx_graph[t].edges:
        ...
        if (
            edge not in fluxes.keys()
            and (edge[1], edge[0]) not in fluxes.keys()
        ):
            fluxes[edge] = 0
