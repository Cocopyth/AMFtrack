...
def activation_continuity(G, node):
    ...
    bool1 = (
        edges[0][2]["post_hyperedge_activation"]
        == edges[1][2]["post_hyperedge_activation"]
    )
...
def simplify(nx_graph):
    ...
    degree_2_nodes = [
        ...
        if nx_graph.degree(node) == 2 and activation_continuity(nx_graph, node)
    ]
    list_attributes = [
        "post_hyperedge_activation",
        "hyperedge",
        "activation",
    ...
    while len(degree_2_nodes) > 0:
        ...
        for node in degree_2_nodes:
            ...
            if nx_graph.degree(node) == 2:
                ...
                if np.any(
                    np.round(right_edge[0])
                    != np.round(np.array((pos[node].x, pos[node].y)))
                ):
                    right_edge = list(reversed(right_edge))
                if np.any(
                    np.round(left_edge[-1])
                    != np.round(np.array((pos[node].x, pos[node].y)))
                ):
                ...
                for attribute in list_attributes:
                    right_edge_attribute = nx_graph.get_edge_data(node, right_n)[
                        attribute
                    ]
                    left_edge_attribute = nx_graph.get_edge_data(node, left_n)[
                        attribute
                    ]
                ...
                for i in range(nx_graph.max_age + 1):
                    ...
                    if (weight_left + weight_right) > 0:
                        attribute_new = (
                            right_edge_attribute * weight_right
                            + left_edge_attribute * weight_left
    ...
    degree_0_nodes = [node for node in nx_graph.nodes if nx_graph.degree(node) <= 1]
...
def get_growing_nodes(exp, index0, index1):
    ...
    subgraph_age_0 = nx.Graph(
        [e for e in G.edges(data=True) if e[2]["post_hyperedge_activation"] <= index0]
    )
    subgraph_age_1 = nx.Graph(
        [
            e
            for e in G.edges(data=True)
            if e[2]["post_hyperedge_activation"] <= index1
            and e[2]["post_hyperedge_activation"] > index0
        ]
    )
...
def convert_to_directed(undirected_graph, radius_attribute="radius"):
    ...
    for u, v, attr in directed_graph.edges(data=True):
        ...
        attr["pot"] = (
            8 * mu * attr["length"] * attr["v0"] / (np.pi * attr[radius_attribute] ** 2)
        )
...
def add_flows_heaton(G, nodes_source, nodes_sink, weights, index):
    for edge in G.edges:
        G[edge[0]][edge[1]]["radius"] = max(
            G[edge[0]][edge[1]][str(index)]["width"] / 2, 1
        )
    ...
    ext_flows_in = {
        node_source.label: -weights[node_source] for node_source in nodes_source
    }  # external flows: positive into the network, negative out
    for node in nodes_sink:
        ext_flows_in[node.label] = 0
    tot_flow = np.sum(list(ext_flows_in.values()))
    ext_flows_out = {
        node_sink.label: -tot_flow / len(nodes_sink) for node_sink in nodes_sink
    }  # external flows: positive into the network, negative out
    ...
    for edge in G.edges:
        G[edge[0]][edge[1]]["water_flux_heaton_abs"] = abs(
            G[edge[0]][edge[1]]["water_flux_heaton"]
        )
        G[edge[0]][edge[1]]["speed_heaton"] = G[edge[0]][edge[1]][
            "water_flux_heaton"
        ] / (np.pi * G[edge[0]][edge[1]]["radius"] ** 2)
...
def add_flows_heaton2(G, nodes_source, nodes_sink, weights, index):
    ...
    for node in nodes_sink:
        DG.add_edge(
            node.label, "ground", length=0, radius=2, v0=0, Res=0, pot=0
        )  # Add edge back to the root
...
def add_backflows(G, nodes_sink, index):
    ...
    for edge in G.edges:
        ...
        G[edge[0]][edge[1]]["speed_backflow"] = (
            2
            * G[edge[0]][edge[1]]["water_flux"]
            / (np.pi * G[edge[0]][edge[1]]["radius"] ** 2)
            - G[edge[0]][edge[1]]["v0"]
        )
...
def add_backflows2(G, nodes_sink, index):
    for edge in G.edges:
        ...
        G[edge[0]][edge[1]]["v0"] = (
            G[edge[0]][edge[1]]["speed_heaton"] / backflow_factor
        )
    DG = convert_to_directed(G)
    for node in nodes_sink:
        DG.add_edge(
    ...
    for edge in G.edges:
        ...
        G[edge[0]][edge[1]]["speed_backflow2"] = (
            2
            * G[edge[0]][edge[1]]["water_flux2"]
            / (np.pi * G[edge[0]][edge[1]]["radius"] ** 2)
            - G[edge[0]][edge[1]]["v0"]
        )
...
def add_backflows_phase(G, nodes_sink, index):
    for edge in G.edges:
        ...
        G[edge[0]][edge[1]]["tot_cross_section"] = (
            np.pi * G[edge[0]][edge[1]]["radius"] ** 2
        )
        G[edge[0]][edge[1]]["lipid_phase_fraction"] = (
            np.abs(G[edge[0]][edge[1]]["speed_heaton"]) * C_factor_flux / lipid_fraction
        )
        G[edge[0]][edge[1]]["lipid_cross_section"] = (
            G[edge[0]][edge[1]]["lipid_phase_fraction"]
            * G[edge[0]][edge[1]]["tot_cross_section"]
        )
        assert G[edge[0]][edge[1]]["lipid_phase_fraction"] < 1
        assert G[edge[0]][edge[1]]["lipid_phase_fraction"] >= 0
        G[edge[0]][edge[1]]["water_cross_section"] = (
            1 - G[edge[0]][edge[1]]["lipid_phase_fraction"]
        ) * G[edge[0]][edge[1]]["tot_cross_section"]
        G[edge[0]][edge[1]]["water_phase_radius"] = np.sqrt(
            (G[edge[0]][edge[1]]["water_cross_section"] / np.pi)
        )

        G[edge[0]][edge[1]]["v0"] = np.sign(G[edge[0]][edge[1]]["speed_heaton"]) * v0
        G[edge[0]][edge[1]]["net_water_flux_phase_lipid"] = (
            G[edge[0]][edge[1]]["lipid_cross_section"]
            * G[edge[0]][edge[1]]["v0"]
            * (1 - lipid_fraction)
        )
    ...
    for edge in G.edges:
        G[edge[0]][edge[1]]["net_water_flux_phase_abs"] = abs(
            G[edge[0]][edge[1]]["net_water_flux_phase"]
        )
        G[edge[0]][edge[1]]["net_water_flux_phase_backflow"] = (
            G[edge[0]][edge[1]]["net_water_flux_phase"]
            + G[edge[0]][edge[1]]["water_flux_heaton"]
            - G[edge[0]][edge[1]]["net_water_flux_phase_lipid"]
        )
        G[edge[0]][edge[1]]["speed_backflow_phase"] = (
            2
            * G[edge[0]][edge[1]]["net_water_flux_phase_backflow"]
            / G[edge[0]][edge[1]]["water_cross_section"]
            - G[edge[0]][edge[1]]["v0"]
...
def add_lipid_flux(G, nodes_source, nodes_sink, weights):
    ...
    for node1 in nodes_sink:
        shortest = nx.single_source_dijkstra_path(G, node1.label, weight="length")
    ...
    for edge in edge_flux.keys():
        if edge[0] > edge[1]:
            G[edge[0]][edge[1]]["QBC_p"] = edge_flux[
                edge
            ]  # Attribute for edge from A to B
        else:
            G[edge[0]][edge[1]]["QBC_m"] = edge_flux[
                edge
            ]  # Attribute for edge from A to B
    for edge in G.edges:
        G[edge[0]][edge[1]]["QBC_net"] = (
            G[edge[0]][edge[1]]["QBC_p"] - G[edge[0]][edge[1]]["QBC_m"]
        )
        G[edge[0]][edge[1]]["QBC_tot"] = (
            G[edge[0]][edge[1]]["QBC_p"] + G[edge[0]][edge[1]]["QBC_m"]
        )
...
def plot_region(exp0, region, attribute, vmax):
    ...
    for edge in edges_network:
        ...
        if is_in_bounding_box(pixels[0], region) or is_in_bounding_box(
            pixels[-1], region
        ):
            if len(pixels) > 100:
...
def plot_arrows_along_edge(ax, begin, end, color, spacing=60):
    ...
    for i in range(1, num_arrows + 1):
        ...
        ax.annotate(
            ...
            xy=(point[1] + unit_vector[1], point[0] + unit_vector[0]),
...
def add_fluxes(exp, index0, index1, folders, num_sinks=20):
    ...
    weights = {
        (begin, end): LineString(data["pixel_list"]).length * 1.725
        for begin, end, data in spatial_temporal_graph.edges(data=True)
    }
    nx.set_edge_attributes(spatial_temporal_graph, weights, "length")
    weights, nodes_growing = get_growing_nodes(exp, index0, index1)
    nodes_source = [
        node for node in nodes_growing if weights[node] / (np.pi * r0**2) * 3600 > 10
    ]
    # nodes_source = [node for node in nodes_source if weights[node] / (np.pi * r0 ** 2) * 3600 <= 1000]
    G0 = create_subgraph_by_attribute(
        spatial_temporal_graph, "post_hyperedge_activation", index0
    )
    components = nx.connected_components(G0)
    combined_graph = SpatialGraph(
        nx.Graph()
    )  # Initialize an empty graph to combine all components
    for component in components:
        ...
        if len(component) > 10:
            ...
            nodes_sink = [
                node
                for node in nodes
                if get_min_activation(component_graph, node.label) <= index0
            ]
            ...
            add_lipid_flux(
                exp1.nx_graph[0], nodes_source_component, nodes_sink, weights
            )
            add_flows_heaton2(
                exp1.nx_graph[0], nodes_source_component, nodes_sink, weights, index0
            )
