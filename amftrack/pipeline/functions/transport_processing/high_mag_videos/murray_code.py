...
p_extend = 0.4  # probability that a tip will extend
p_split = 0.1  # probabiity that a tip will split into two tips
...
def add_seg(network, node, end_coords):
    ...
    nnodes += 1
    network = {'conn': conn, 'coords': coords, 'nnodes': nnodes}
    return network


def make_self_connection(network, tip_node, old_node):
    '''Add connection from tip_node to old_node to connectivity matrix.'''
    conn = network['conn']
    conn[tip_node, old_node] = 1
    network['conn'] = conn
    return network


def apply_tip_dels(tip_dels, tip_nodes, directions):
    '''Remove nodes in tip_dels from tip_nodes and directions.'''
    for i in range(len(tip_dels)):
        idx = tip_nodes.index(tip_dels[i])
        tip_nodes.pop(idx)
        directions.pop(idx)
    return (tip_nodes, directions)


def extend_tip(network, tip_nodes, tip_dels, directions, grids, tip):
    ...
    # Add new node
    new_node_coords = [tip_coords[0] + dldt * np.sin(new_dir),
    ...
    if in_grid(row, col) and obs_grid[row, col] == 0:
        if fungus_grid[row, col] == 0:  # check for possible self connection
            ...
            network = add_seg(network, tip_node, new_node_coords)
            directions[tip] = new_dir
            fungus_grid[row, col] = new_node_no  # update fungus grid
        elif fungus_grid[row, col] != tip_node:  # self-connect if cell occupied by another node
            old_node = fungus_grid[row, col]
            network = make_self_connection(network, tip_node, old_node)
            tip_dels.append(tip_node)
    grids['fungus'] = fungus_grid
    return (network, tip_dels, directions, grids)


def split_tip(network, tip_nodes, tip_dels, directions, grids, tip):
    ...
    new_node_coords1 = [tip_coords[0] + dldt * np.sin(new_dir - split_angle),
                        tip_coords[1] + dldt * np.cos(new_dir - split_angle)]
    ...
    if in_grid(row, col) and obs_grid[row, col] == 0:
        if fungus_grid[row, col] == 0:  # check for self connection
            ...
            network = add_seg(network, tip_node, new_node_coords1)
            directions[tip] = new_dir - split_angle
    ...
    new_node_coords2 = [tip_coords[0] + dldt * np.sin(new_dir + split_angle),
                        tip_coords[1] + dldt * np.cos(new_dir + split_angle)]
    ...
    if in_grid(row, col) and obs_grid[row, col] == 0:
        if fungus_grid[row, col] == 0:  # check for self connection
            ...
            network = add_seg(network, tip_node, new_node_coords2)
            directions.append(new_dir + split_angle)
    ...
    for t in range(t_growth):
        ...
        for tip in range(len(tip_nodes)):
            if np.random.uniform(0, 1) < p_split:  # split the tip into two
                (network, tip_dels, directions, grids) = split_tip(network, tip_nodes,
                                                                   tip_dels,
                                                                   directions, grids, tip)
            elif np.random.uniform(0, 1) < p_extend:  # grow the tip without splitting
                (network, tips_dels, directions, grids) = extend_tip(network, tip_nodes,
                                                                     tip_dels,
                                                                     directions, grids, tip)
        # Delete tips that made self-connections
        (tip_nodes, directions) = apply_tip_dels(tip_dels, tip_nodes, directions)
        # Plot the extended network and add to list of frames for animation
        frame = plot_network_frame(network, tip_nodes)
        frames.append(frame)
    # Save animation as gif
    frame_one = frames[0]
    frame_one.save('growth_animation.gif', format="GIF", append_images=frames,
                   save_all=True, duration=80)
    return (network, tip_nodes)
...
def update_flow_gradient(network, carbon_surplus):
    ...
    for node in range(nnodes):
        conn_row = conn[node]
        neighbours = np.where(conn_row == 1)[0]
        neighbours = [i for i in neighbours if carbon_surplus[node] > carbon_surplus[i]]  # outgoing flow only
...
def simulate_carbon_flow(t_flow, network, tip_nodes, carbon_load, flow_gradient, carbon_status):
    # First frame for animation
    frame = plot_carbon_frame(network, tip_nodes, carbon_load, flow_gradient, carbon_status, 0)
    frames = [frame]
    ...
    for time in range(t_flow):
        ...
        # Updates
        carbon_load = update_carbon_load(network, carbon_load, flow_gradient, carbon_status)
        frame = plot_carbon_frame(network, tip_nodes, carbon_load, flow_gradient, carbon_status, time + 1)
        frames.append(frame)
        # Save animation as gif
    frame_one = frames[0]
    frame_one.save('carbon_animation.gif', format="GIF", append_images=frames,
                   save_all=True, duration=80)
...
def plot_network(network, tip_nodes):
    ...
    img = grids['obstacle']
    ax.matshow(img, extent=[xmin, xmax, ymin, ymax], origin='lower', cmap='Greys')
    for i in range(nnodes):
        xfrom = coords[i, 0]
        yfrom = coords[i, 1]
...
def plot_carbon_frame(network, tip_nodes, carbon_load, flow_gradient, carbon_status, time):
    ...
    for i in range(nnodes):
        ...
        c = Circle(coords[i], radius=0.2 * carbon_load[i], clip_on=False, linewidth=1,
