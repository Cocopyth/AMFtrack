...
from amftrack.pipeline.functions.transport_processing.high_mag_videos.register_videos import check_hasedges
import os
import numpy as np
import imageio.v2 as imageio

font_size = 24  # Change this to your desired font size
font = ImageFont.truetype("/usr/share/fonts/dejavu/DejaVuSansMono.ttf", font_size)  # 'arial.ttf' might need to be adjusted based on your system
    ...
    for vid_obj in data_obj.video_objs:
        if vid_obj.dataset['mode'] == "BF":
            ...
            for edge in vid_obj.edge_objs:
                ...
                draw.text(start_pos, str(begin), fill="white", font=font)  # , font=font) if using a custom font
                draw.text(end_pos, str(end), fill="white", font=font)  # , font=font) if using a custom font
