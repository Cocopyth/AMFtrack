    ...
    for edge in edge_objs:
        ...
        kymo_tiff = np.array(
            [
                ...
                edge.filtered_right[0],
            ],
            dtype=np.int16,
        )
        imwrite(
            f"{edge.edge_path}/{edge.edge_name}_kymos_array.tiff",
            kymo_tiff,
            photometric="minisblack",
        )

        spd_tiff = np.array(
            [edge.speeds_tot[0][0], edge.speeds_tot[0][1], edge.flux_tot], dtype=float
        )
        imwrite(
            f"{edge.edge_path}/{edge.edge_name}_speeds_flux_array.tiff",
            spd_tiff,
            photometric="minisblack",
        )
        ...
        vel_adj = np.where(
            np.isinf(np.divide(spd_tiff[2], kymo_tiff[1])),
            np.nan,
            np.divide(spd_tiff[2], kymo_tiff[1]),
        )
        ...
        data_table = {
            ...
            "flux_coverage": 1
            - np.count_nonzero(np.isnan(edge.flux_tot), axis=1) / len(edge.flux_tot[0]),
            "speed_left_coverage": 1
            - np.count_nonzero(np.isnan(edge.speeds_tot[0][0]), axis=1)
            / len(edge.flux_tot[0]),
            ...
            [
                {
                    ...
                    "speed_right": np.nanmean(
                        np.nanmean(edge.speeds_tot[0][1], axis=1)
                    ),
                    "speed_mean": np.nanmean(vel_adj_mean),
                    "speed_left_std": np.nanstd(
                        np.nanmean(edge.speeds_tot[0][0], axis=1)
                    ),
                    "speed_right_std": np.nanstd(
                        np.nanmean(edge.speeds_tot[0][1], axis=1)
                    ),
...
def plot_summary(edge_objs, spd_max_percentile=99.5):
    for edge in edge_objs:
        ...
        speedmax = np.max(
            [np.nanpercentile(abs(spd_tiff[0:2].flatten()), spd_max_percentile), 5]
        )
        ...
        speed_histo_left = np.array(
            [np.histogram(row, speed_bins)[0] for row in edge.speeds_tot[0][0]]
        )
        speed_histo_right = np.array(
            [np.histogram(row, speed_bins)[0] for row in edge.speeds_tot[0][1]]
        )
        speed_histo = (speed_histo_left + speed_histo_right) / (
            2 * len(edge.speeds_tot[0][0][0])
        )

        fig, ax = plt.subplot_mosaic(
            [
                ["kymo", "speed_hist_zoom", "speed_hist"],
                ["speed_plot", "flux_plot", "speed_hist"],
            ],
            figsize=(12, 8),
            layout="constrained",
        )
        ...
        imshow_extent = [
            0,
            space_res * edge.kymos[0].shape[1],
            time_res * edge.kymos[0].shape[0],
            0,
        ]
        ...
        ax["speed_plot"].fill_between(
            edge.times[0],
            np.nanmean(edge.speeds_tot[0][0], axis=1)
            + np.nanstd(edge.speeds_tot[0][0], axis=1),
            np.nanmean(edge.speeds_tot[0][0], axis=1)
            - np.nanstd(edge.speeds_tot[0][0], axis=1),
            alpha=0.5,
            facecolor="tab:blue",
        ...
        ax["speed_plot"].fill_between(
            edge.times[0],
            np.nanmean(edge.speeds_tot[0][1], axis=1)
            + np.nanstd(edge.speeds_tot[0][1], axis=1),
            np.nanmean(edge.speeds_tot[0][1], axis=1)
            - np.nanstd(edge.speeds_tot[0][1], axis=1),
            alpha=0.5,
        ...
        ax["speed_hist"].imshow(
            speed_histo.T,
            extent=[0, len(speed_histo) * time_res, -speedmax, speedmax],
            origin="lower",
            aspect="auto",
            cmap=hist_cmap,
        )
        ...
        ax["speed_hist_zoom"].imshow(
            speed_histo.T[speed_bins_trunc],
            extent=[0, len(speed_histo) * time_res, -7, 7],
            origin="lower",
            aspect="auto",
            cmap=hist_cmap,
        )
        ...
        ax["flux_plot"].plot(
            edge.times[0],
            np.nanmean(edge.flux_tot, axis=1),
            c="black",
            label="Average flux",
        )
        ax["flux_plot"].fill_between(
            edge.times[0],
        ...
        fig, ax = plt.subplot_mosaic(
            [
                ["kymo", "kymo_left", "kymo_right"],
                ["kymo_stat", "spd_left", "spd_right"],
            ],
            figsize=(12, 9),
            layout="constrained",
        )
        ax["kymo"].imshow(
            kymo_tiff[0], cmap="gray", vmin=0, aspect="auto", extent=imshow_extent
        )
        ax["kymo_stat"].imshow(
            kymo_tiff[1], cmap="gray", aspect="auto", extent=imshow_extent
        )
        ax["kymo_right"].imshow(
            kymo_tiff[2], cmap="gray", aspect="auto", extent=imshow_extent
        )
        ax["kymo_left"].imshow(
            kymo_tiff[3], cmap="gray", aspect="auto", extent=imshow_extent
        )
        ax["spd_left"].imshow(
            spd_tiff[0],
            cmap="coolwarm",
        ...
        spd_colors = ax["spd_right"].imshow(
            spd_tiff[1],
            cmap="coolwarm",
            vmin=-speedmax,
            vmax=speedmax,
