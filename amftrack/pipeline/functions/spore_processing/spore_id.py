...
def find_corresp(spore_position_t,spore_position_tp1,thresh = 30):
    ...
    for sporet in spore_position_t.keys():
        for sporetp1 in spore_position_tp1.keys():
            if np.linalg.norm(spore_position_t[sporet]-spore_position_tp1[sporetp1])<=thresh:
    ...
    for sporet in spore_position_t.keys():
        if len(corresp[sporet])>0:
            ...
            for sporetp1 in corresp[sporet]:
                dist = np.linalg.norm(spore_position_t[sporet]-spore_position_tp1[sporetp1])
                if dist<mini:
...
def create_labeled_spores(corresp_list, spore_positions_dic, spore_radius_dic,folders):
    ...
    for t in range(len(corresp_list) + 1):
        ...
        final_positions[t] = {mapping(spore): spore_positions_dic[t][spore] for spore in spore_positions_dic[t].keys()}
        final_radiuses[t] = {mapping(spore): spore_radius_dic[t][spore] for spore in spore_radius_dic[t].keys()}

    spore_data_total = pd.DataFrame()
    for t in range(len(corresp_list) + 1):
        spore_dataf = pd.concat((pd.DataFrame(final_positions[t], index=['x', 'y']).transpose(),
                                 pd.DataFrame(final_radiuses[t], index=['radius']).transpose()), axis=1)
        spore_dataf['t'] = t
...
def make_spore_data(exp):
    ...
    for t in range(exp.ts - 1):
        ...
        positionstp1 = {i:
                            exp.timestep_to_general(np.flip(spore_data[i, :2]), tp1)
                        for i in range(len(spore_data))
                        }
        corresp_list.append(find_corresp(positionst, positionstp1))
        folders[t] = {"folder" : table['folder'].iloc[t],"datetime" : table['datetime'].iloc[t]}
    ...
    spore_radiuses_dic[t] = radiusest
    folders[t] = {"folder" : table['folder'].iloc[t],"datetime" : table['datetime'].iloc[t]}
    spore_data_total = create_labeled_spores(corresp_list, spore_positions_dic, spore_radiuses_dic,folders)
    return (spore_data_total)