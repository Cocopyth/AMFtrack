...
def get_curvature_density_bait(window, path,kern_sizes=[]):
    ...
    for treatment in ['25*','25','baits']:
        ...
        for inst in insts:
            ...
            imageio.mimsave(f'/home/cbisot/pycode/MscThesis/amftrack/notebooks/plotting/Figure/movie_dense_{kern_size}_{plate_number[inst]}_thresh.gif', img_array,duration = 1)
            for index,density_map in enumerate(density_maps):
                plt.close('all')
                fig=plt.figure(figsize=(14,12))
                ax = fig.add_subplot(111)
            ...
            for index in range(len(density_maps)):
                img = cv2.imread(f'/home/cbisot/pycode/MscThesis/amftrack/notebooks/plotting/Figure/im*{index}.png')
                img_array.append(img)
            imageio.mimsave(f'/home/cbisot/pycode/MscThesis/amftrack/notebooks/plotting/Figure/movie_dense_{kern_size}_{plate_number[inst]}.gif', img_array,duration = 1)
            skeletons = [sparse.csr_matrix(skel) for skel in exp.skeletons]
