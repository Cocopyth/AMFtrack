...
for treatment in ["25", "baits"]:
    ...
    for inst in insts:
        bait_positions[inst] = pickle.load(
            open(f"{path_code}/MscThesis/Results/baits_{inst}.pick", "rb")
        )
...
def get_curvature_density_bait(window, path, kern_sizes=[]):
    ...
    for treatment in ["25*", "25", "baits"]:
        ...
        for inst in insts:
            ...
            density_maps = [
                get_density_maps(exp, t, compress, kern_sizes) for t in range(exp.ts)
            ]
            ...
            for hyph in RH:
                for i, t in enumerate(hyph.ts[:-1]):
                    ...
                    speed = np.sum([get_length_um(seg) for seg in segs]) / get_time(
                        exp, t, tp1
                    )
                    ...
                    vy = v_vector[1]
                    dist_P = (
                        np.linalg.norm(np.array(pos) - np.array(pos_P))
                        * pixel_conversion_factor
                    )
                    dist_N = (
                        np.linalg.norm(np.array(pos) - np.array(pos_N))
                        * pixel_conversion_factor
                    )
                    x, y = pos[0], pos[1]
                    ...
                    if np.linalg.norm(vec1) > 0 and np.linalg.norm(vec2) > 0:
                        ...
                        if (
                            ...
                            new_line = pd.DataFrame(new_line_dic)  # index 0 for
...
pickle.dump(
    infos, open(f"{path_code}/MscThesis/Results/straight_bait_{window}.pick", "wb")
)
