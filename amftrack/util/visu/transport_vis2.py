...
# analysis_frame
video_paths = [os.path.join(analysis_folder,row['plate_id'],row['video_int'],"Img",f"{row['plate_id']}_{row['video_int']}_video.mp4") for index,row in analysis_frame.iterrows()]
...
detected_edges_path = [os.path.join(analysis_folder,row['plate_id'],row['video_int'],"Img",f"Detected edges.png") for index,row in analysis_frame.iterrows()]
...
def generate_dash_leaflet_app(image_path, video_pos, video_list,assets_folder="asset"):
    ...
    video_list = ["/assets/"+os.path.relpath(video, assets_folder).replace('\\','/') for video in video_list]
    print(video_list[0])
    ...
    app.layout = html.Div([
        dl.Map(children=[
            dl.ImageOverlay(url=f"/assets/{image_path}", bounds=[[0, 0], [image_height, image_width]]),
        ...
        html.Div(
            [html.Video(src=video, controls=True, id=f"video-{i}", style={'width': '50%', 'display': 'none'},preload = None) for i, video in enumerate(video_list)]
        )
    ])
    @app.callback(
    ...
    def display_video(*clicks):
        ...
        styles = [{'display': 'block' if i == marker_index else 'none', 'width': '50%'} for i in range(len(video_list))]
        return styles
