...
app.layout = html.Div([
    ...
    html.Div([
        dcc.Dropdown(
            ...
            options=[{'label': i, 'value': i} for i in df_sum['treatment'].dropna().unique()],
            value=df_sum['treatment'].dropna().unique()[0]
        ),
        dcc.Dropdown(
            id='root-dropdown',
            options=[{'label': i, 'value': i} for i in df_sum['root'].dropna().unique()],
            value=df_sum['root'].dropna().unique()[0]
        ),
        dcc.Dropdown(
            id='fungus-dropdown',
            options=[{'label': i, 'value': i} for i in df_sum['fungus'].dropna().unique()],
            value=df_sum['fungus'].dropna().unique()[0]
        ),
        dcc.Checklist(
            id='suitable-for-coarse-grain-checklist',
            options=[{'label': i, 'value': i} for i in df_sum['Suitable for coarse grain'].dropna().unique()],
            inline=True,
            value=df_sum['Suitable for coarse grain'].dropna().unique().tolist()
        ),
    ...
    html.Div([
        # Video Player
        html.Div(id='video-player', style={'margin-bottom': '20px', 'width': '100%'}),

        # Graph Controls + Graph
        html.Div([
            dcc.Dropdown(id='xaxis-column', style={'width': '45%', 'display': 'inline-block'}),
            dcc.Dropdown(id='yaxis-column', style={'width': '45%', 'display': 'inline-block', 'margin-left': '10px'}),
            dcc.Graph(id='data-graph'),
            dash_table.DataTable(id='table', style_table={'height': '300px', 'overflowY': 'auto'})
        ], style={'flex': '1', 'width': '100%'})
    ], style={'flex': '2', 'width': '68%', 'flex-direction': 'column'})
], style={'display': 'flex', 'justify-content': 'space-between', 'padding': '10px', 'flex-direction': 'row'})
...
def filter_videos_for_list(treatment, root, fungus, coarse_grain_values):
    ...
    return html.Ul([html.Li(html.A(uid, href='#', id={'type': 'video-link', 'uid': uid})) for uid in video_filenames])
...
def play_video(clicks):
    ...
    analysis_folder = next((folder for folder in os.listdir(os.path.join(root_dir, analysis_dir, str(uid))) if
                            folder.startswith("Analysis_")), None)

    if analysis_folder:
        json_path = os.path.join(root_dir, analysis_dir, str(uid), analysis_folder, 'time_plate_info.json')
    ...
    return html.Video(src=video_path, controls=True, style={'width': '100%'}), df.to_dict(
        'records'), column_options, column_options
