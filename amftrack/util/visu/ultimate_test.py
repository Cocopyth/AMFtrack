...
def generate_dash_leaflet_app():
    ...
    app.layout = html.Div(
        [
            dcc.Store(
                id="display-state", data={"show_map": False}
            ),  # Using Store to manage display state
            html.Div(
                [
                ...
                id="content-div",
            ),  # Setting initial content to the selection screen
        ]
    )
    ...
    def update_display(data):
        if data["show_map"]:
            return html.Div(
                [
                    dl.Map(
                        children=dl.ImageOverlay(
                            url=image_overlay_url, bounds=[[0, 0], [5587, 11379]]
                        ),
                        id="map",
