...
all_folders_origin['date_datetime'] =  pd.to_datetime(all_folders_origin['date'].astype(str),format = "%d.%m.%Y, %H:%M:")
selection = (datetime.now()-all_folders_origin['date_datetime'])<=timedelta(hours = 15)
last_folders = all_folders_origin.loc[selection]
plates_to_stitch = last_folders['Plate'].unique()

for plate in plates_to_stitch:
    folders = all_folders_origin.loc[all_folders_origin['Plate']==plate].sort_values('date_datetime',ascending = False).head(1)    
